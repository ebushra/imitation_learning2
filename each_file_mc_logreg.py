'''
This file runs rollouts (using Logistic Regression) on each file in the MountainCar data section and reports data such as
...
def rollout_model(model, scaler, episodes=25):
    ...
    for ep in range(episodes):
        ...
        # MountainCar success = reaching the flag (episode ends early)
        if steps < 200:
            successes += 1
...
for f in files:
    ...
    try:
        ...
        print("\nLogReg accuracy:", accuracy)

        y_proba = model.predict_proba(X_test)
        logreg_val_loss = log_loss(y_test, y_proba)

        noisiness = logreg_val_loss

        print("LogReg validation loss (noisiness):", logreg_val_loss)
        ...
        results[os.path.basename(f)] = {
            ...
            "logreg_val_loss": float(logreg_val_loss),
